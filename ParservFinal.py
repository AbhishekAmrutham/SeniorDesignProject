...
"""
Data Stucture: List of Lists, Each element of the "big list" has an IP along with a list of vulnerabilities under of vulernabilities, Each Vulnerability is a Dictionary with CVSS Info
NOTE: Notice that once the statistic for exploitability given 100% is added to the "big list", we only look at the vectors to the second to last element. 
Hence, the code for IP in nessusData[:-1]:
"""
    ...
    for report in root.iter("Report"):
        for IP in report.iter("ReportHost"):
            ...
            for vulnerability in IP.iter("cvss_vector"):
                ...
                # count the number of vulnerabilities when first parsing the file
                global numberOfVulnerabilities
...
def writeParsedData(nessusData, filename):
    with open(filename, mode='w+') as writer:
        ...
        csvWriter.fieldnames = ["IP", "CVSS Base Vector", "Posture", "Access Vector", "Access Vector Score",
                                "Access Complexity", "Access Complexity Score", "Authentication",
                                "Authentication Score", "Confidentiality Impact", "Confidentiality Impact Score",
                                "Integrity Impact", "Integrity Impact Score", "Availability Impact",
                                "Availability Impact Score", "Exploitability %", "Affected Metrics", "Statement"]
...
# Function to parse base vectors
...
# Convert CVSS letters to numbers
def getCVSSValues(parsedVector):
    # Use dictionaries to parse
    ...
    # Store the affected Metrics to an array
    affectedMetrics = []
    ...
    parsedVector["Affected Metrics"] = ",".join(affectedMetrics); #add array to parsed vector
...
def generatePlot(nessusData, filename):
    ...
    possibleExploitabilities = [0, 3.47, 8.02, 9.03, 10.56, 14.62, 14.72, 16.62, 21.67, 21.77, 22.16, 24.55, 26.30,
                                30.57, 30.90, 32.95, 36.21, 42.09, 44.48, 48.51, 49.18, 58.79, 59.57, 63.84, 76.64,
                                83.91, 100.0]
...
# check if there exists an element in a that is an element of b
def likeElement(a,b):
    return not set(a).isdisjoint(b)

# check if IpAddress is valid format
...
# a simple user check for yes or no
...
# function to get posture filters from user
...
# function to create a filename depending on the 3 categories of filters(postures, impacts, IPs)
...
# function to take input and ask for filters and perform filtering
def customFilter(nessusData, path):
    filterPostures = list()
    filterImpacts = list()
    # Get info from user
    # Custom filter?
    ...
    while True:
        ...
        if custom_filter == 'y':
            ...
            while True:
                ...
                startProcessing = time.time()  # processing of filtering starts here

                # Create Filter Data File Name
                filterFileName = createFileName(filterPostures, filterImpacts, filterIPs)

                # Create new copy of data for filtering
                ...
                removeEmptyIPs(data)
                endProcessing = time.time()  # store end of filtering time
                global processingTime
                processingTime += (endProcessing - startProcessing)  # add to processing time
                ...
                if len(data) == 0:
                    ...
                    startProcessing = time.time()  # processing of writing data starts here
                    calculateStats(data)
                    writeParsedData(data, path + '/' + filterFileName + '.csv')
                    generatePlot(data, path + '/' + filterFileName + '.png')
                    endProcessing = time.time()  # store end of data write time
                    processingTime += (endProcessing - startProcessing)  # add to processing time
                    break
...
# function to handle empty IP addresses(no vulnerabilities under IP)
def removeEmptyIPs(nessusData):
    for IP in nessusData[:-1]:
        if (len(IP) == 1):
            nessusData.remove(IP)

# function to filter data by posture
...
# Function to filter data by impact
def filterDataByImpact(nessusData, impact):
    for IP in nessusData[:-1]:
        for Vulnerability in IP[1:]:
            # 3 variables to store if the vulnerability affects either of those 3 criteria
            # These variables will be false if we aren't filtering for those criteria

            # if vulnerability is concerned with then availability must be affected
            availability = impact[0] and Vulnerability["Availability Impact"] != 'N'

            # if vulnerability is concerned with then confidentiality must be affected
            confidentiality = impact[1] and Vulnerability["Confidentiality Impact"] != 'N'

            # if vulnerability is concerned with then integrity must be affected
            integrity = impact[2] and Vulnerability["Integrity Impact"] != 'N'

            # if the vulnerability is not in either of the categories we are interested in, then remove it
            if not(availability or confidentiality or integrity):
                IP.remove(Vulnerability)

# Function to create scan folder with the timestamp of the scan
...
# create variables to hold processing time and number of vulnerabilities
processingTime = 0  # processing time is time when program is working and not waiting for user input
numberOfVulnerabilities = 0
start = time.time()  # store start time
FILENAME = getFile()
# initial processing
startProcessing = time.time()  # initial processing starts when parsing XML
data = parse_xml(FILENAME)
path = createScanFolder()
endProcessing = time.time()  # initial processing ends when creating folder for test results
processingTime += (endProcessing-startProcessing)  # add initial processing time to total time
customFilter(data, path)  # working through custom filters
# do final processing with writing final output scan(full network)
startProcessing = time.time()
writeParsedData(data, path + "/FullNetworkScan.csv")
generatePlot(data, path + '/FullNetworkScan.png')
endProcessing = time.time()
processingTime += (endProcessing-startProcessing)  # add final processing time
end = time.time()  # store end time
elapsedTime = end-start  # calculate total elapsed time of program
userEntryTime = elapsedTime-processingTime  # calculate user entry time
# print out all relevant data regarding program execution
print("Number of Vulnerabilities: %d" % numberOfVulnerabilities)
print("Processing Time: %fs" % processingTime)
print("Elapsed Time: %fs" % elapsedTime)
print("User Entry Time: %fs" % userEntryTime)
