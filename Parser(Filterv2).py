...
def filterByPosturesPrompt():
    filterPostures = list()
    custom_filter = input("Would you like to filter by postures? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
        filter_setting = input("How would you like to filter?\n"
        ...
        while (True):
            ...
            # Filter by Nearsider
            elif filter_setting == '3':
                filterPostures.append("Nearsider")

            # Filter by Outsider
            elif filter_setting == '4':
                filterPostures.append("Outsider")
            # No more postures or invalid input
            else:
                break
            filter_setting = input("Would you like to add another posture?\n"
                                   "1 Privileged Insider\n"
                                   "2 Insider\n"
                                   "3 Nearsider\n"
                                   "4 Outsider\n"
...
def filterByImpactsPrompt():
    ...
    filterImpacts.append(False)
    filterImpacts.append(False)
    custom_filter = input("Would you like to filter by impacts? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
        filter_setting = input("How would you like to filter?\n"
                               "1 Availability\n"
                               "2 Confidentiality\n"
        ...
        while (True):
            ...
            # No more postures or invalid input
                ...
                break
            filter_setting = input("Would you like to add another impact?\n"
                                   "1 Availability\n"
                                   "2 Confidentiality\n"
                                   "3 Integrity\n"
                                   "4 No\n")
...
def filterByIPsPrompt():
    filterIPs = list()
    custom_filter = input("Would you like to filter by IPs? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
        filter_setting = input("How would you like to filter?\n"
                               "1 Single IP\n"
                               "2 Range of IPs\n")
...
def customFilterPrompt(nessusData):
    filterPostures = list()
    filterImpacts = list()
    # Get info from user
    # Custom filter?
    custom_filter = input("Would you like to create a custom report? (Y/N)")
    ...
    if custom_filter == 'Y' or custom_filter == 'y':
        ...
        writeParsedData(data, filterFileName)
...
def createScanFolder():
    ts = time.time()
    st = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d_h%H-m%M-s%S')
