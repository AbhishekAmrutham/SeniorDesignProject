...
'''
Data Structure: List of IPs, with List of Vulnerabilities. Each Vulnerability is a Dictionary with CVSS Info
Note: Once the statistic for vulnerability given 100% attacker capability is found it is appended to the List Of IPs, this is why when looking at the IP list 
we look at all the IPs except for the last element(the statistic). This is the code: for IP in nessusData[:-1]:
'''
...
def writeParsedData(nessusData, filename):
    with open(filename, mode='w+') as writer:
        ...
        for IP in nessusData[:-1]:
            ...
            #if filters are being used and the IP size is 1 don't print row(IP.size)
...
#function to map values frome letters to numbers and add them to the parsed data
...
def filterByPosturesPrompt():
    filterPostures = list()
    custom_filter = input("\nWould you like to filter by postures? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
...
def filterByImpactsPrompt():
    ...
    filterImpacts.append(False)
    filterImpacts.append(False)
    custom_filter = input("\nWould you like to filter by impacts? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
        filter_setting = input("\n\n\nHow would you like to filter?\n"
                               "(Seperate Values with a comma)\n"
                               "(Enter in all values before pressing enter)\n"
...
def filterByIPsPrompt():
    filterIPs = list()
    custom_filter = input("\nWould you like to filter by IPs? (Y/N)")
    if custom_filter == 'Y' or custom_filter == 'y':
        filter_setting = input("\n\n\nHow would you like to filter?\n"
                               "1 Single IP\n"
                               "2 Range of IPs\n")
...
def customFilter(nessusData, path):
    ...
    # Get info from user
    # Custom filter?
    custom_filter = input("Would you like to create a custom filter? (Y/N)")
    ...
    while (True):
        ...
        custom_filter = input("Would you like to create an additional custom filter? (Y/N)")
...
customFilter(data, path) #does calculations and creates reports based on user criteria
# measure time of  writing data and plot
startProcessing = time.time()
...
userEntryTime = elapsedTime-processingTime;
